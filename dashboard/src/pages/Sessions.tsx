...
export function Sessions() {
  ...
  useWebSocket({
    onSessionStatus: useCallback(
      (event: { sessionId: string; status: string; phone?: string }) => {
        setSessions(prev =>
          prev.map(s => (s.id === event.sessionId ? { ...s, status: event.status as Session['status'], phone: event.phone || s.phone } : s)),
        );
        if (event.status === 'ready') {
          setQrData(null); // Close QR modal when ready
          toast.success(t('sessions.toasts.readyTitle'), t('sessions.toasts.readyDesc'));
        } else if (event.status === 'authenticated') {
          setQrData(null); // Close QR modal when authenticated
