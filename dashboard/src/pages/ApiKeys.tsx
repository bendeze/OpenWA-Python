...
import { useApiKeysQuery, useCreateApiKeyMutation, useDeleteApiKeyMutation, useRevokeApiKeyMutation, useRotateApiKeyMutation } from '../hooks/queries';
...
export function ApiKeys() {
  ...
  const [confirmAction, setConfirmAction] = useState<{ type: 'delete' | 'revoke' | 'rotate'; id: string; name: string } | null>(
    null,
  );
  ...
  const handleRotate = async (id: string) => {
    try {
      ...
      setCreatedKey(rotated.fullKey || rotated.apiKey || null);
      setShowModal(true);
    } catch (err) {
  ...
  const confirmAndExecute = () => {
    ...
    else if (confirmAction.type === 'revoke') handleRevoke(confirmAction.id);
    else if (confirmAction.type === 'rotate') handleRotate(confirmAction.id);
        ...
        cell: info => {
          ...
          return (
            <span className="key-cell">
              <code>{visibleKeys.has(apiKey.id) ? apiKey.fullKey : apiKey.keyPrefix + '****'}</code>
          ...
          return (
            <span className="actions-cell">
              ...
              {apiKey.isActive && (
                <button
                  className="icon-btn"
                  onClick={() => setConfirmAction({ type: 'rotate', id: apiKey.id, name: apiKey.name })}
                  title={t('apiKeys.actions.rotate')}
                >
                  <KeyRound size={16} />
                </button>
              )}
